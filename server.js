...
app.get("/", (req, res) => {
  res.render("index", { plants: null, message: null }); // Initial load with no data
});
...
app.get("/getPlants", async (req, res) => {
  ...
  const watering = params.water_schedule
    ? "&watering=" + params.water_schedule
    : "";
  const sunlight = params.sunlight ? "&sunlight=" + params.sunlight : "";
  const apiUrl =
    `https://perenual.com/api/species-list?key=${process.env.PERENUAL_API_KEY}&indoor=1` +
    edible +
    poisonous +
    cycle +
    watering +
  ...
  try {
    ...
    if (validData && validData.length > 0) {
      ...
      res.render("index", {
        plants: [],
        message:
          "No results returned, please modify your selection and try again.",
      });
    }
  } catch (error) {
